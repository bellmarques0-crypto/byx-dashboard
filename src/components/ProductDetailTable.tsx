...
// Converte valor vindo do Sheets (ISO ou YYYY-MM-DD) para o formato do input date (YYYY-MM-DD)
...
// Valor do input date já vem como YYYY-MM-DD (ótimo para salvar no Sheets)
function fromDateInputValue(value: string) {
  return value || "";
}

export const ProductDetailTable: React.FC<Props> = ({
  productId,
  vagas,
  candidates,
  onUpdateCandidate,
  ...
  const renderCell = (index: number, field: keyof Candidate, type = "text") => {
    ...
    return (
      <input
        ...
        value={value}
        onChange={(e) =>
          onUpdateCandidate(productId, index, field, e.target.value)
        }
        placeholder="..."
    ...
    return (
      <div className="relative group w-full">
        <select
          value={value}
          onChange={(e) =>
            onUpdateCandidate(productId, index, "horarioTrabalho", e.target.value)
          }
      ...
      <div className="overflow-x-auto custom-scrollbar">
        <table className="w-full text-[12px] text-left border-collapse whitespace-nowrap">
          <thead className="bg-[#4a5f9e] text-white uppercase font-bold text-[11px]">
            <tr>
              <th className="px-4 py-4 border-r border-white/10 text-center w-20 sticky left-0 bg-[#4a5f9e] z-10">
                ITEM
              </th>
              <th className="px-6 py-4 border-r border-white/10 min-w-[500px]">
                NOME DO CANDIDATO
              </th>
              <th className="px-6 py-4 border-r border-white/10 min-w-[220px]">
                HORÁRIO TRABALHO
              </th>
              <th className="px-6 py-4 border-r border-white/10 min-w-[360px]">
                INSTRUTOR
              </th>
              <th className="px-6 py-4 border-r border-white/10 min-w-[220px]">
                HORÁRIO CURSO
              </th>
              <th className="px-6 py-4 border-r border-white/10 min-w-[200px] text-emerald-100 bg-emerald-800/50 text-center">
                INÍCIO CURSO
              </th>
              <th className="px-6 py-4 border-r border-white/10 min-w-[200px] text-center">
                FIM CURSO
              </th>
              <th className="px-6 py-4 border-r border-white/10 min-w-[200px] text-center">
                ASO
              ...
              <th className="px-6 py-4 border-r border-white/10 min-w-[220px] text-center">
                INÍCIO NA OPERAÇÃO
              </th>
              <th className="px-6 py-4 border-r border-white/10 min-w-[300px]">
                OBS
              </th>
            </tr>
          </thead>
          ...
          <tbody>
            {Array.from({ length: vagas }).map((_, idx) => (
              <tr
                ...
                className={
                  idx % 2 === 0
                    ? "bg-blue-50/60"
                    : "bg-white hover:bg-blue-100/40 transition-colors"
                }
              >
                ...
                <td className="px-6 py-3 border-b border-emerald-100 bg-emerald-50/50 font-black text-black text-center">
                  <input
                    type="date"
                    value={toDateInputValue(candidates[idx]?.inicioCurso ?? "")}
                    onChange={(e) =>
                      onUpdateCandidate(
                        productId,
                        idx,
                ...
                <td className="px-6 py-3 border-b border-gray-100 font-bold text-black text-center">
                  <input
                    type="date"
                    value={toDateInputValue(candidates[idx]?.fimCurso ?? "")}
                    onChange={(e) =>
                      onUpdateCandidate(
                        productId,
                        idx,
            ...
            {vagas === 0 && (
              <tr>
                <td
                  colSpan={11}
                  className="px-8 py-32 text-center text-black font-bold text-xl italic bg-gray-50/50"
                >
